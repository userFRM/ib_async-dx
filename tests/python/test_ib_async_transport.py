...
def test_every_call_their_library_makes_is_carried():
    ...
    unrouted = [
        ...
        and not hasattr(ibkr_dx.EClient, ib_async_dx.bridge._our_name_for(name, ibkr_dx.EClient))
    ]
...
def test_a_contract_named_by_id_states_nothing_else():
    ...
    """
    by_id = ib_async_dx.bridge._as_ours(ib_async.Contract(conId=756733))
    ...
    described = ib_async_dx.bridge._as_ours(ib_async.Contract(symbol="AAPL"))
    ...
    stock = ib_async_dx.bridge._as_ours(ib_async.Stock("AAPL", "SMART", "USD"))
...
def test_a_combination_keeps_its_legs_across_the_bridge():
    ...
    ours = ib_async_dx.bridge._as_ours(theirs)
...
def test_what_tunes_an_algo_reaches_the_order():
    ...
    ours = ib_async_dx.bridge._as_ours(their)
