...
dependency. The one class that differs is `IB`, a subclass of ib_async's whose
`connect` takes a login instead of a gateway's address. The rest of the program
— its events, its `*Async` calls, its `Ticker` and `Trade`, its `util` — runs
unmodified, with the engine in place of the one layer that knew there was a
socket.
...
<p>No login window to script and no gateway to restart when it wedges. The engine logs in itself, and rebuilds a dropped connection on the session it already holds.</p>
</div>
</div>
...
<p><code>ib_async_dx</code> exports ib_async's 103 names, and every one but <code>IB</code> and <code>__version__</code> is ib_async's own object. A test holds the two packages to that on every run.</p>
</div>
<div class="dx-card">
...
<a class="dx-card" href="./notebooks.html"><strong>The notebooks</strong><span>Seven of ib_async's eight notebook subjects, run with no gateway.</span></a>
...
Under active development. The Python package is here with its suite: 51 tests,
49 of which run offline on every push to `main` and every pull request, and 2
that need a live login. ib_async's own suite runs against the engine too, where
